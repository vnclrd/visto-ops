...
export const StoreSelectionPage: React.FC<StoreSelectionProps> = ({
  ...
  const sortedStores = [...rawStores].sort((a, b) => {
    ...
    // Primary sort: Active vs Inactive
    if (aActive !== bActive) {
      return Number(bActive) - Number(aActive);
    }

    // Secondary sort: Oldest created timestamp first
  ...
  const openStorePinModal = (store: StoreItem) => {
    // Prevent opening modal if store is inactive
    ...
    <div className='flex min-h-screen items-center justify-center bg-neutral-950 px-4 select-none relative'>
      <div className='w-full max-w-lg rounded-2xl bg-neutral-900 p-8 border border-neutral-800 shadow-2xl'>
        ...
        <p className='text-xs text-neutral-400 mb-6'>
          Logged in as{' '}
          <span className='text-neutral-200 font-semibold'>{account.name}</span>
        </p>
        ...
        {sortedStores.length === 0 ? (
          ...
          <div className='space-y-3'>
            {sortedStores.map((store) => {
              const active = store.isActive ?? true;
              ...
              return (
                <button
                  ...
                  disabled={!active}
                  className={`w-full flex items-center justify-between p-4 rounded-xl border transition text-left group ${
                    active
                      ? 'border-neutral-800 bg-neutral-950/60 hover:bg-neutral-800 hover:border-neutral-700 cursor-pointer'
                ...
                >
                  <div>
                    <div className='flex items-center gap-2'>
                      ...
                      {!active && (
                        <span className='text-[10px] font-medium tracking-wide uppercase px-2 py-0.5 rounded bg-neutral-800 text-neutral-400 border border-neutral-700'>
                          Inactive
                        </span>
                      )}
                    </div>
